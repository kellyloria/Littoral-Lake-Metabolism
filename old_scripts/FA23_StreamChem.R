...
streamchem <- read.csv("/Users/kellyloria/Downloads/Processedwaterchemistry.csv", header=T)
str(streamchem)
...
SCHEM1 <- SCHEM %>%
  mutate(shore = case_when(
    Site %in% c("Blackwood lower", "Blackwood upper", "Blackwood 0.5m", "Blackwood outlet") ~ "west",
    Site %in% c("Glenbrook lower", "Glenbrook upper", "Glenbrook 0.5m", "Glenbrook outlet") ~ "east",

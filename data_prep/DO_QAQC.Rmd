---
title: Data aggregation of all miniDOT data from the Near shore of Lake Tahoe cinder
  blocks
author: "Kelly Loria"
date: '2023-03-02'
output:
...
GBNS1 <- read.csv("./AggRawData/AggregatedDO/GBNS1_v2.csv", header=T)
GBNS2 <- read.csv("./AggRawData/AggregatedDO/GBNS2_v2.csv", header=T)
GBNS3 <- read.csv("./AggRawData/AggregatedDO/GBNS3_v2.csv", header=T)

## MiniDOT sensors - Blackwood
BWNS1 <- read.csv("./AggRawData/AggregatedDO/BWNS1_v2.csv", header=T)
BWNS2 <- read.csv("./AggRawData/AggregatedDO/BWNS2_v2.csv", header=T)
BWNS3 <- read.csv("./AggRawData/AggregatedDO/BWNS3_v2.csv", header=T)
...
miniDOT_adj <- function(x){
  ...
  df <- x %>% dplyr::select(Pacific.Standard.Time, Dissolved.Oxygen, Temperature, site) %>%
    dplyr::rename(datetime = Pacific.Standard.Time, do.obs = Dissolved.Oxygen, wtr = Temperature, site = site) %>%
...
GS1_adj <- GS1_adj[-which(GS1_adj$datetime >= "2021-06-15 13:00:00" & GS1_adj$datetime <= "2021-06-17 13:00:00"),]
vis_data_DO(GS1_adj)
...
GS3_adj <- GS3_adj[-which(GS3_adj$datetime >= "2021-06-15 13:00:00" & GS3_adj$datetime <= "2021-06-17 13:00:00"),]
vis_data_DO(GS3_adj)
...
vis_data_DO(BS1_adj)
BS1_adj <- BS1_adj[-which(BS1_adj$datetime >= "2021-10-14 10:00:00" & BS1_adj$datetime <= "2021-10-15 17:00:00"),]
vis_data_DO(BS1_adj)
BS1_adj <- BS1_adj[-which(BS1_adj$datetime >= "2021-06-15 13:00:00" & BS1_adj$datetime <= "2021-06-17 13:00:00"),]
vis_data_DO(BS1_adj)
...
BS2_adj <- BS2[-which(BS2$datetime >= "2021-06-15 00:00:00" & BS2$datetime <= "2021-06-17 12:00:00"),]
vis_data_DO(BS2_adj)

BS2_adj <- BS2_adj[-which(BS2_adj$datetime >= "2021-10-14 00:00:00" & BS2_adj$datetime <= "2021-10-15 18:00:00"),]
vis_data_DO(BS2_adj)
# none
```
...
BS3_adj <- BS3_adj[-which(BS3_adj$datetime >= "2021-06-15 00:00:00" & BS3_adj$datetime <= "2021-06-17 12:00:00"),]
vis_data_DO(BS3_adj)

BS3_adj <- BS3_adj[-which(BS3_adj$datetime >= "2021-10-14 00:00:00" & BS3_adj$datetime <= "2021-10-15 18:00:00"),]
vis_data_DO(BS3_adj)

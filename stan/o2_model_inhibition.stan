...
transformed data {
  ...
  // scale data (standariza los valores no endiendo bien porque)
  mu = mean(o2_obs); 
  tau = sd(o2_obs);
  eta = mean(light);
  ...
  for (n in 1:n_obs){
    x_obs[n] = (o2_obs[n] - mu)/tau; #standarizacion de el valor de O2 obs
    x_eq[n] = (o2_eq[n] - mu)/tau; #standarizacion de el valor de O2 eq
    lambda[n] = light[n]/eta;# tipo de standarizacion de el valor de O2 eq (nose porque no se divide por sd)
...
model {
  ...
  // daily parameters # smooth calculation from previous day
  {
    int pos = 1;
    for (y in 1:n_years) {
      // inital value
...
>>>>>>> 4ca41570d122768c1b1e94ed065b72f7445b0d38
